...
body=f'''<section class="page-head map-head"><div class="wrap"><p class="eyebrow">地方建設與公開紀錄</p><h1>建設與進度</h1><p class="map-intro">查找地方建設，閱讀每個階段的公開紀錄。</p></div></section><section class="wrap map-controls" aria-label="建設紀錄篩選"><div><label for="case-search">關鍵字</label><input id="case-search" type="search" placeholder="搜尋道路、學校、寵物或案件名稱"></div><div><label for="village-filter">里別／服務範圍</label><select id="village-filter"><option value="all">全部里別與範圍</option>{opts}</select></div><details class="advanced-filters"><summary>進階篩選：主題、進度與年度</summary><div class="advanced-filter-fields"><div><label for="category-filter">主題</label><select id="category-filter"><option value="all">全部主題</option>{''.join(f'<option>{E(t)}</option>' for t in cats)}</select></div><div><label for="subcategory-filter">小分類</label><select id="subcategory-filter"><option value="all">全部小分類</option>{''.join(f'<option>{E(t)}</option>' for t in subcats)}</select></div><div><label for="status-filter">進度</label><select id="status-filter"><option value="all">全部進度</option>{''.join(f'<option>{E(t)}</option>' for t in statuses)}</select></div><div><label for="year-filter">歷程年度</label><select id="year-filter"><option value="all">全部年度</option>{year_options}<option value="undated">未載歷程年度</option></select></div></div></details><button type="button" id="reset-map-filters">清除篩選</button><p class="case-live-summary" role="status">目前收錄 {len(public_items)} 個專題 · <a href="#case-results">查看結果 ↓</a></p></section><div class="wrap civic-view-controls" role="group" aria-label="紀錄顯示方式"><span>閱讀方式</span><button type="button" data-view="both" aria-pressed="false" disabled>地圖與列表</button><button type="button" data-view="list" aria-pressed="true" disabled>只看列表</button></div><section class="wrap map-workspace civic-list-only"><div class="map-panel" hidden><a class="text-link" href="#case-results">跳到篩選結果 ↓</a><div id="achievement-map" role="region" aria-label="鳳山政績互動地圖"><p class="map-startup">地圖載入中，所有紀錄也可由下方列表閱讀。</p></div><p class="map-message" id="map-message" role="status">點選里界可篩選；數字標記代表附近專題數，放大可分開查看。</p><p class="map-source">里界：{ext('https://data.gov.tw/dataset/7438','內政部國土測繪中心')}，2026-08-17。點位為代表位置，不是施工範圍；全市政策與尚缺座標的紀錄只列於列表。</p><button type="button" id="map-fit" class="map-fit">查看目前結果範圍</button></div><div class="case-results" id="case-results"><div class="results-heading"><h2>建設與服務紀錄</h2><p id="case-count" aria-live="polite">共 {len(public_items)} 個專題 · 第 1/{(len(public_items)+9)//10} 頁 · {sum(bool(c["coordinates"]) for c in public_items)} 個有點位紀錄</p></div><div id="case-list">{''.join(card(c) for c in public_items)}</div><div id="case-empty" hidden><h3>這個條件下目前沒有顯示紀錄。</h3><p>請調整篩選條件，或查看全部公開紀錄。</p><button type="button" data-clear-filters>顯示全部紀錄</button></div></div></section><noscript><p class="wrap">目前未啟用 JavaScript，地圖與篩選暫不可用；下方完整紀錄與詳情頁仍可直接閱讀。</p></noscript><script type="application/json" id="map-data">{json.dumps({'url':map_data_url},ensure_ascii=False)}</script>'''
...
page('achievements.html','建設與進度｜鳳山公開紀錄','查詢鳳山建設與民眾服務專題，依里別、主題、小分類與進度查詢政績、服務與推動歷程。',body,head)
