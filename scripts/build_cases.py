...
def tags(c):return ''.join(f'<span>{E(t)}</span>' for t in c['categories']+c['villages'])
...
for c in items:
 ...
 photos=''
 if c['images']:photos='<div class="case-photos">'+''.join(f'<a href="assets/{E(p)}"><img src="assets/{E(p)}" alt="{E(c["title"])}公開照片" loading="lazy"></a>' for p in c['images'])+'</div>'
 ...
 if c['budget']:info+=f'<div><dt>原紀錄經費</dt><dd>{E(c["budget"])}</dd></div>'
 info+='</dl>'
 note=''
 if c['notes']:note='<aside class="case-notes"><h2>紀錄補充</h2><ul>'+''.join('<li>'+E(t)+'</li>' for t in c['notes'])+'</ul></aside>'
 sources='<p class="source-caption">內容依服務處公開紀錄整理；狀態保留來源所載階段，尚未取得後續證明的項目不標示完工。</p>'
 if c['sources']:sources+='<ul class="source-links">'+''.join('<li>'+ext(x['url'],x['title'])+'</li>' for x in c['sources'])+'</ul>'
 if c['coordinates']:sources+=f'<p class="source-caption">地圖使用公開紀錄中的代表位置；里界依內政部國土測繪中心2026年8月17日圖資。{E(c["villageMethod"])}</p>'
 related=''
 ...
 page(href(c['id']),c['title'],c['summary'],body,'<script type="application/ld+json">'+json.dumps(structured,ensure_ascii=False).replace('<','\\u003c')+'</script>')
...
opts=''.join(f'<option value="v:{E(v)}">{E(v)}</option>' for v in villages)+''.join(f'<option value="s:{E(s)}">{E(s)}</option>' for s in scopes)+'<option value="unassigned">里別待補</option>'
cats=sorted({t for c in items for t in c['categories']});statuses=sorted({c['status'] for c in items})
mapdata=[{k:c[k] for k in ['id','title','summary','categories','villages','scope','status','coordinates']} for c in items]
body=f'''<section class="page-head map-head"><div class="wrap"><p class="eyebrow">FENGSHAN, ONE PLACE AT A TIME</p><h1>慧做事・政績地圖</h1><p>從你的里出發，看看地方建設、服務與每一步進度。</p><div class="map-stats"><span><strong>{len(items)}</strong> 個專題紀錄</span><span><strong>{len(villages)}</strong> 里可點選</span><span>點選地圖或卡片，閱讀站內完整歷程</span></div></div></section><section class="wrap map-controls" aria-label="政績篩選"><div><label for="case-search">關鍵字</label><input id="case-search" type="search" placeholder="搜尋道路、學校或案件名稱"></div><div><label for="village-filter">里別／服務範圍</label><select id="village-filter"><option value="all">全部里別與範圍</option>{opts}</select></div><div><label for="category-filter">主題</label><select id="category-filter"><option value="all">全部主題</option>{''.join(f'<option>{E(t)}</option>' for t in cats)}</select></div><div><label for="status-filter">進度</label><select id="status-filter"><option value="all">全部進度</option>{''.join(f'<option>{E(t)}</option>' for t in statuses)}</select></div><button type="button" id="reset-map-filters">清除篩選</button></section><section class="wrap map-workspace"><div class="map-panel"><div id="achievement-map" role="region" aria-label="鳳山政績互動地圖"><p class="map-startup">地圖載入中，所有紀錄也可由下方列表閱讀。</p></div><p class="map-message" id="map-message" role="status">點選里界可篩選；數字標記代表該位置收錄的專題數。</p><p class="map-source">里界：{ext('https://data.gov.tw/dataset/7438','內政部國土測繪中心')}，2026-08-17。點位為代表位置，不是施工範圍；全市政策與尚缺座標的紀錄只列於列表。</p><button type="button" id="map-fit" class="map-fit">查看目前結果範圍</button></div><div class="case-results"><div class="results-heading"><h2>建設與服務紀錄</h2><p id="case-count" aria-live="polite">共 {len(items)} 個專題</p></div><div id="case-list">{''.join(card(c) for c in items)}</div><div id="case-empty" hidden><h3>這個條件下還沒有收錄紀錄。</h3><p>這不代表該里沒有服務或建設；你可以更換條件，或查看全市政策。</p><button type="button" data-clear-filters>顯示全部紀錄</button></div></div></section><noscript><p class="wrap">目前未啟用 JavaScript，地圖與篩選暫不可用；下方完整紀錄與詳情頁仍可直接閱讀。</p></noscript><script type="application/json" id="map-data">{json.dumps(mapdata,ensure_ascii=False).replace('<','&#60;')}</script>'''
head='<link rel="stylesheet" href="assets/vendor/leaflet.css"><link rel="stylesheet" href="map.css?v=20260908-4"><script src="assets/vendor/leaflet.js" defer></script><script src="map.js?v=20260908-4" defer></script>'
page('achievements.html','政績地圖','結合鳳山里界、建設位置、主題與進度，直接在本站閱讀政績說明及歷史紀錄。',body,head)
