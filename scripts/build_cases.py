...
for c in items:
 ...
 article='<article class="case-body">'+content+photos+history+'</article>' if content or photos or history else ''
 ...
 body=f'''<div class="wrap breadcrumb"><a href="./">首頁</a><span>/</span><a href="achievements.html">政績地圖</a><span>/</span><span>{E(c['title'])}</span></div><section class="page-head case-head"><div class="wrap"><p class="eyebrow">WORK & PROGRESS</p><div class="case-tags">{tags(c)}</div><h1>{E(c['title'])}</h1>{('<p>'+E(c['summary'])+'</p>') if c['summary'] else ''}</div></section><div class="wrap {layout_class}">{article}<aside class="case-aside">{info}<a class="button button-green" href="achievements.html?case={E(c['id'])}">{'在地圖查看' if c['coordinates'] else '回到政績列表'} →</a><a class="text-link" href="petition.html">有相關問題想反映 →</a></aside></div>{related}'''
...
head='<link rel="stylesheet" href="assets/vendor/leaflet.css"><link rel="stylesheet" href="map.css?v=20260908-nav2"><script src="assets/vendor/leaflet.js" defer></script><script src="map.js?v=20260908-nav2" defer></script><script type="application/ld+json">'+json.dumps([collection,breadcrumbs],ensure_ascii=False).replace('<','\\u003c')+'</script>'
page('achievements.html','鳳山政績與建設追蹤','查詢鳳山建設與民眾服務專題，依里別、主題與進度閱讀來源、位置與歷史紀錄。',body,head)
