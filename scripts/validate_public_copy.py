...
public_achievement_pages = list(ROOT.glob("achievement-*.html"))
if len(public_achievement_pages) != 19:
    failures.append(f"expected 19 public achievement pages, found {len(public_achievement_pages)}")
...
if failures:
    ...
    for item in failures:
        print("-", item)
    raise SystemExit(1)
print(f"Public copy validation passed: {len(html_files)} HTML files; {len(public_achievement_pages)} public achievements")
