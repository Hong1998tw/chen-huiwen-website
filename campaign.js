...
(() => {
  ...
  function renderElectionDates(data) {
    ...
    const update = () => {
      ...
      if (unit) unit.textContent = diff > 0 ? '天，距離投票日' : '投票日到了';
    };
    update();
  ...
  async function init() {
    ...
    if (platforms.status === 'fulfilled') renderPlatforms(platforms.value); else if (q('#campaign-platforms')) q('#campaign-platforms').innerHTML = '<p class="campaign-empty">政見資料暫時無法載入，請改至歷屆政見頁查看。</p>';
    if (achievements.status === 'fulfilled') renderTracking(achievements.value); else if (q('#campaign-tracking')) q('#campaign-tracking').innerHTML = '<p class="campaign-empty">追蹤資料暫時無法載入，請改至政績頁查看。</p>';
    if (events.status === 'fulfilled') renderEvents(events.value); else if (q('#campaign-events')) q('#campaign-events').innerHTML = '<p class="campaign-empty">公開行程暫時無法載入，請改至活動公告查看。</p>';
    installSearch();
  }
