...
- Cloudflare Rocket Loader／bot challenge 可能針對 GitHub hosted headless browser；live browser test 保持 `https://www.huiwen.tw/` 為頁面 URL，並由本機唯讀 Python live proxy 使用與 HTTP parity 相同的固定 verifier UA 向 Production edge 取得同源 response，再交由 Chromium 執行互動／Accessibility。這不是 checkout mock；若 Production response、runtime 或必要 asset 持續失敗，gate 仍必須 Fail。有限 retry 與 `edgeRetries` 只處理暫時性 edge challenge。
