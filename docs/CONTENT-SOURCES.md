...
- 政績圖片必須是該專題的直接證據，並在 `data/achievements.json` 同時填入 `images`、`imageDimensions` 與 `imageMetadata`；`alt`、`caption`、`credit`、`sourceUrl` 缺一即不得 build。
- 新聞圖片只接受同一事件且權利可追溯的素材。由前端資料掛載的圖片必須保留固定來源 URL 與 credit；媒體記者、通訊社、議會攝影或權利未明圖片只保留原文連結，不下載、不重製。
- 同一照片若同時用於新聞與政績，兩個位置都要各自說明照片日期、事件及其與該頁的關係，不得把後續追蹤照寫成原事件現場照。
- 政績頁以搜尋、篩選、地圖與逐案證據為主要入口，不顯示衍生件數、主題分布、進度分布或年度統計總覽；需要統計時另立經審核的資料產品。
- Facebook、Canva 等第三方 iframe 可能因供應商或瀏覽器設定失敗，需保留直接連結作 fallback。
- Facebook iframe 一律採使用者主動載入；載入前不得預留 600px 空白區，首頁與新聞頁沿用同一 opt-in 行為及 fallback 文案。
