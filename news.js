...
(() => {
  ...
  const filterPanel = section.querySelector('[data-news-filters]');
  const search = section.querySelector('#news-search');
  const sort = section.querySelector('#news-sort');
  const status = section.querySelector('[data-news-count]');
  const empty = section.querySelector('[data-news-empty]');
  const pagination = section.querySelector('[data-news-pagination]');
  ...
  const cards = [...grid.querySelectorAll('[data-news-categories]')].map((node, index) => {
    const source = node.querySelector('.card-body > .eyebrow')?.textContent || node.textContent;
    const match = source.match(/(20\d{2})[.\/-](\d{2})[.\/-](\d{2})/);
    const date = match ? `${match[1]}-${match[2]}-${match[3]}` : '0000-00-00';
    return {node, date, categories:(node.dataset.newsCategories || '').split(/\s+/).filter(Boolean), text:String(node.textContent || '').toLocaleLowerCase('zh-Hant-TW'), order:index};
  });
  ...
  const makeButton = (text, page, options = {}) => {
    ...
    button.type = 'button'; button.className = 'news-page-button'; button.textContent = text;
    button.disabled = Boolean(options.disabled);
    if (options.current) button.setAttribute('aria-current', 'page');
    button.setAttribute('aria-label', options.label || `新聞報導第 ${page} 頁`);
  ...
  const renderPagination = (total, pages) => {
    ...
    if (total <= PAGE_SIZE) { pagination.hidden = true; return; }
  ...
  const render = () => {
    ...
    grid.replaceChildren(...matches.slice(start,start+PAGE_SIZE).map(item => item.node));
