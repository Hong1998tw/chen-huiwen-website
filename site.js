...
(() => {
  ...
  // Generated by scripts/build_shared.py from the dependency bytes, before hashing site.js.
  const DIGITAL_SCRIPT_VERSION = '6fa530c18d78';
  const DIGITAL_STYLE_VERSION = '7ffb5403c98e';
  const scriptURL = new URL(`digital.js?v=${DIGITAL_SCRIPT_VERSION}`, HUIWEN_ASSET_BASE);
  const styleURL = new URL(`digital.css?v=${DIGITAL_STYLE_VERSION}`, HUIWEN_ASSET_BASE);
  const sameAsset = (value, target) => {
    if (!value) return false;
    const url = new URL(value, document.baseURI);
    return url.origin === target.origin && url.pathname === target.pathname;
  };
  // An explicitly declared asset is authoritative even without a legacy data marker.
  if (![...document.querySelectorAll('link[rel="stylesheet"]')].some(link => sameAsset(link.href, styleURL))) {
  ...
  if (![...document.scripts].some(script => sameAsset(script.src, scriptURL))) {
