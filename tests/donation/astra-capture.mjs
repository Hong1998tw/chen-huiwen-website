...
for(const width of [1440,390]){
 ...
 for(const route of ['index.html','achievements.html','achievement-metro-green-line.html','service.html','news.html','petition.html','explore.html','404.html']){
  await page.goto('http://127.0.0.1:8766/'+route);await page.waitForTimeout(250);await page.mouse.move(width-1,1);
  ...
  if(route==='index.html'){
   ...
   await page.locator('.civic-stories').evaluate(el=>el.scrollIntoView({block:'start',behavior:'instant'}));await page.screenshot({path:`${out}/${label}-reading-${width}.png`});
  }
  if(route==='achievements.html'){
   ...
   await page.mouse.move(width-1,1);await page.locator('.map-panel').screenshot({path:`${out}/${label}-map-${width}.png`});
   await page.getByRole('button',{name:'只看列表',exact:true}).click();await page.locator('#case-results').evaluate(el=>el.scrollIntoView({block:'start',behavior:'instant'}));await page.screenshot({path:`${out}/${label}-list-${width}.png`});
  }
 }
}
await browser.close();
