...
try{
 ...
 await check('timeline-visible-with-inactive-observer',async()=>{
  const c=await localContext({viewport:{width:390,height:844}});await c.addInitScript(()=>{window.IntersectionObserver=class{observe(){}unobserve(){}disconnect(){}};});const p=await c.newPage();await p.goto(base+'achievement-metro-green-line.html');const items=p.locator('.case-timeline > li');assert((await items.count())>0);assert(await items.evaluateAll(xs=>xs.every(x=>getComputedStyle(x).opacity!=='0'&&getComputedStyle(x).visibility!=='hidden')));await c.close();
 });
